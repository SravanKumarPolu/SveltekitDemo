...
<div class="navbar fixed top-0 left-0 right-0 z-50 bg-base-100 shadow-lg">
	...
	<div class=" lg:flex items-center space-x-4">
		...
		>
			<!-- Filter Display -->
			<div
				class="h-[6rem] overflow-x-auto md:overflow-x-hidden cursor-pointer bg-base-100 rounded-lg overflow-y-auto scrollbar-thin"
			>
				<!-- Display selected filters in the navbar -->
				<div
					class="flex flex-col sm:flex sm:gap-2 sm:w-auto h-auto mb-2 md:h-[8rem] w-[8rem] md:w-[18.8rem] sm:space-y-1 md:grid md:grid-cols-4 md:gap-0 lg:grid-cols-4 font-bold"
				>
					<!-- Item 1 (Customers) -->
					...
					<!-- Item 2 (Carriers) -->
					<div
						class="text-sm mb-1 flex flex-col col-span-2 space-y-1 col-start-3 row-1"
					>
						<div class="flex-grow relative group">
							<div
								class="truncate max-w-xs"
					...
					<div
						class="text-sm flex mb-1 flex-col col-span-2 col-start-3 space-y-1"
					>
						<div class="flex-grow relative group">
							<div
								class="truncate max-w-xs"
