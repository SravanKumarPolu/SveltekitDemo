<script>
  ...
  const filteredReports = derived(
    ...
    ([$downloadReportsList, $statusFilter, $isScheduledFilter]) => {
      ...
      // Filter by 'Scheduled' or 'On Demand'
      if ($statusFilter.includes("Scheduled")) {
        reports = reports.filter((report) => report.isScheduled === true);
      } else if ($statusFilter.includes("On Demand")) {
        reports = reports.filter((report) => report.isScheduled === false);
      }

      // Further filter by other statuses (if not 'All')
  ...
  function handleStatusChange(status) {
    statusFilter.update((filters) => {
      ...
      if (status === "Scheduled") {
        isScheduledFilter.set(true); // Activate 'Scheduled' filter
      } else if (status === "On Demand") {
        isScheduledFilter.set(false); // Activate 'On Demand' filter
      }

      // Ensure that 'Scheduled' and 'On Demand' are mutually exclusive
      if (status === "Scheduled" || status === "On Demand") {
        updatedFilters = updatedFilters.filter(
          (f) => f !== "Scheduled" && f !== "On Demand",
        );
      }
...
<div class="card w-full bg-base-100 m-4  shadow-xl">
  <!-- Card Header -->

  <!-- Header Section with Background -->
  <div
    class="card-header bg-base-100 p-2 w-full rounded-lg flex flex-col items-center shadow-md"
  >
      ...
      <div class="flex justify-center space-x-4">
        ...
        <label class="inline-flex items-center space-x-2">
          <input
            type="checkbox"
            class="checkbox checkbox-primary"
            on:change={() => handleStatusChange("Scheduled")}
        ...
        <label class="inline-flex items-center space-x-2">
          <input
            type="checkbox"
            class="checkbox checkbox-primary"
            on:change={() => handleStatusChange("On Demand")}
            checked={!$isScheduledFilter && $statusFilter.includes("On Demand")}
          />
          <span>On Demand</span>
        </label>
      </div>
  ...
  <div class="card-body">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {#each $filteredReports.slice(($currentPage - 1) * $itemsPerPage, $currentPage * $itemsPerPage) as report, index (report.id)}
        <div
          class="card h-[280px] bg-gradient-to-r from-white via-gray-100 to-gray-50 shadow-xl rounded-md transition-transform transform hover:scale-105 duration-500 ease-in-out border border-gray-300 hover:shadow-2xl"
        >
          <!-- Card Header -->
          <div
            class={`card-header h-20 p-2 text-left rounded-t-md text-primary-content
      ${getStatusStyles(report.status)} flex flex-row items-center`}
          >
            ...
            <div class="relative group w-full">
              <div
                class=" break-words tooltip tooltip-bottom h-auto z-30"
                data-tip={report.fileName}
                style="max-width: 90%; "
              >
                <p class="line-clamp-2 w-full overflow-hidden text-left">
                  {report.fileName}
                </p>
              </div>
            </div>
          ...
          <!-- Card Body -->
          <div class="card-body p-1 text-sm  text-left">
            <div class="flex flex-row justify-between h-auto">
              ...
              {#if !report.isScheduled}
                <!-- Show 'Demand' button only if 'isScheduled' is false -->
                <button class="btn btn-outline btn-primary">Demand</button>
              {/if}
            </div>
            <div class=" ">
              <p class="text-gray-600">
