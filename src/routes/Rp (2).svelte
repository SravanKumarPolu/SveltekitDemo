...
<div class="card shadow-lg p-2 rounded-lg mt-4 border border-gray-400">
	...
	<form on:submit|preventDefault={handleRunReport} class="space-y-6">
		<div
			class="flex flex-col lg:flex-row card-body space-y-5 lg:space-y-0 lg:space-x-5"
		>
			<!-- Left Section -->
				...
				<div class="flex w-full gap-5">
					...
					>
						...
						{#each reportDropDown as report}
							<option value={report.id} class="truncate" title={report.name}>
								{report.name}
							</option>
						{/each}
					</select>
