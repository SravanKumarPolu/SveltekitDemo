<script>
  ...
  onMount(() => {
    ...
    initializeSortableDashboards();
  });

  // Initialize Sortable for dashboards
  const initializeSortableDashboards = () => {
  ...
  // Initialize Sortable for chart IDs
  const initializeSortableChartIds = () => {
    const chartIdListElement = document.getElementById("chart-id-list");
    if (chartIdListElement) {
      new Sortable(chartIdListElement, {
        animation: 150,
        onEnd: (event) => {
          const selectedDashboard = dashboards.find((dash) => dash.id === selectedDashboardId);
          if (selectedDashboard) {
            // Update the order of chartIds
            const [movedItem] = chartIds.splice(event.oldIndex, 1);
            chartIds.splice(event.newIndex, 0, movedItem);

            // Update the dashboard's chartIds
            selectedDashboard.chartIds = [...chartIds];
            saveDashboardsToLocalStorage(); // Save changes
          }
        },
      });
  ...
  const selectDashboard = (dashboardId) => {
    ...
    initializeSortableChartIds(); // Initialize Sortable for chart IDs
  };
...
<div class="flex flex-col  md:flex-row gap-2 mt-2 py-4 ">
  ...
  <div class="w-full bg-base-100 shadow-md rounded-lg p-6">
    ...
    <div class="h-[32rem] md:h-[31rem] overflow-y-auto scrollbar scrollbar-thin">
      <ul id="dashboard-list" class="menu bg-base-200 mt-4 rounded-lg p-2 ">
        {#if filteredDashboards.length > 0}
          {#each filteredDashboards as dashboard}
            ...
            <div class="flex justify-between">
              <span
              class="cursor-pointer text-sm font-medium text-gray-800 truncate flex-grow"
