<script>
 ...
 const loadCategoriesAndReports = () => {
  ...
  onMount(() => {
    ...
    // Initialize Sortable for reports in selected category
    ...
    if (reportListElement) {
      new Sortable(reportListElement, {
        ...
        onEnd: (event) => {
          ...
          if (selectedCategory) {
            const [movedReport] = selectedCategory.reports.splice(event.oldIndex, 1);
   ...
   // Reactively initialize Sortable on reports list when selectedCategoryId changes
...
const cancelCategoryEdit = () => {
  ...
  categories.forEach((category) => {
    tempCategoryNames = { ...tempCategoryNames, [category.id]: category.name };
  });
  editingCategories = false;
};
...
const saveAllReportChanges = () => {
  reports = reports.map((report) => ({
    ...report,
    reportCustomizedName: tempReportNames[report.id] || report.reportCustomizedName,
  }));
...
const cancelReportEdit = () => {
  ...
  reports.forEach((report) => {
    tempReportNames = { ...tempReportNames, [report.id]: report.reportCustomizedName };
  });
  editingReports = false;
};
...
const handleSelectChange = (event) => {
  const selectedIds = Array.from(event.target.selectedOptions).map(option => parseInt(option.value));
  const newTags = selectedIds.map(id => reports.find(report => report.id === id)?.reportCustomizedName);
...
const removeTag = (tag) => {
  const reportToRemove = reports.find(report => report.reportCustomizedName === tag);
  if (reportToRemove) {
    selectedReportIds = selectedReportIds.filter(id => id !== reportToRemove.id);
...
const addReports = () => {
  ...
  const newReportIds = selectedReportIds.filter(id => !existingReports.has(id));
...
<div class="flex flex-col md:flex-row gap-4 mt-4">
 ...
 <ul id="category-list" class="menu bg-base-100 rounded-md overflow-y-auto h-64">
  {#each filteredCategories as category}
    <li class="py-1 flex flex-row items-center">
      ...
      <button class="ml-2" on:click={() => deleteCategory(category.id)}>
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-trash3 text-error" viewBox="0 0 16 16">
  ...
  <div class="w-full md:w-1/2 p-4 border border-gray-500 rounded-lg shadow-lg relative">
    ...
    <div class="dropdown-container w-full md:w-full p-2 rounded-md shadow-md border border-gray-400 relative">
      <!-- Input Field with Tags -->
  <!-- Badge container with scrollable overflow -->
<div class="flex gap-1">
  <div class="input input-bordered w-[90%]  flex  h-auto rounded-md flex-wrap overflow-y-auto scrollbar scrollbar-thin   items-start gap-2 p-2 cursor-pointer" on:click={toggleDropdown} >
    {#if inputTags.length > 0}
      {#each inputTags as tag}
        <div class="badge badge-sm badge-primary text-sm flex  items-center gap-1 whitespace-nowrap">
          {tag}
          <button class="btn btn-xs btn-circle" on:click={() => removeTag(tag)}>✕</button>
        </div>
      {/each}
    {:else}
  ...
  <button class="btn btn-sm h-10 btn-secondary" on:click={addReports}>
    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-plus-square btn-secondary" viewBox="0 0 16 16">
      <path d="M14 1a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H2a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1zM2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2z"/>
      <path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4"/>
    </svg>
  </button>
      ...
      <!-- Dropdown Menu -->
      <div class={`dropdown-content menu p-2 shadow bg-base-100 rounded-box w-full mt-2 ${isDropdownOpen ? 'block' : 'hidden'}`}>
        <select
          multiple
 ...
 {#if selectedCategoryId}
 <div class="w-full md:w-1/2 p-4 border border-gray-500 rounded-lg shadow-lg mt-4">
   <h3 class="text-md font-semibold mb-2">
     Reports for Category {selectedCategoryId}:
   </h3>
   <ul id="report-list" class="menu bg-base-100 rounded-md">
